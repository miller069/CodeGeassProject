...
class Level:
    ...
    def create_map(self):
        ...
        if gid_map:
            ...
            print("[Map] No tilemap found -- using colored fallback tiles.")

        # Fallback tile colors used when tilemap assets are absent.
        # grass tiles use two alternating greens for a subtle checkerboard.
        FALLBACK_COLORS = {
            'grass':  [(34, 120, 34), (44, 140, 44)],
            'object': [(80, 50, 20)],
        }
        _tile_counter = [0]  # mutable list so inner function can mutate it

        def make_fallback_surf(sprite_type):
            colors = FALLBACK_COLORS.get(sprite_type)
            if not colors:
                return None
            color = colors[_tile_counter[0] % len(colors)]
            _tile_counter[0] += 1
        ...
        for csv_path, sprite_type, groups in LAYERS:
            try:
                ...
                for (row, col), tile_id in entries:
                    ...
                    if surf is None and not gid_map:
                        surf = make_fallback_surf(sprite_type)
                    if surf is not None:
